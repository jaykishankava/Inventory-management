...
import Header from './Header'; // Updated import statement
...
function Dashboard() {
  ...
  return (
    <div className="container text-center">
      ...
      <div className="row justify-content-center mt-3">
        {filteredItems.map(item => (
          <div key={item.id} className="col-sm-6 col-md-4 mb-3">
            <div className={`card ${item.quantity < 5 ? 'bg-secondary text-white' : 'bg-info text-white'}`} style={{borderRadius:"15px"}}>
              <div className="card-body">
                <h5 className="card-title">{item.name}</h5>
                <p className="card-text">Quantity: {item.quantity}</p>
                <p className="card-text">Category: {item.category}</p>
                <p className="card-text">Supplier: {item.supplier}</p>
                <button onClick={() => handleEditItem(item)} className="btn btn-success me-2 mx-2 border border-dark">Edit</button>
      ...
      <Link to="/view" className="btn btn-primary mb-5 mt-3" state={{ items }}>View All Items</Link>
    </div>
  );
}
